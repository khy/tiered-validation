...
shared_examples_for 'a model with tiered validation, or one of its subclass' do
  describe 'Validation tier convenience methods' do
    describe '.create_with_[tier]_validation' do
      it 'should create an instance using the specified tier validation' do
        account_class.create_with_user_validation!(:expiration_date => Time.now).should be_a_kind_of(Account)
      end
  
      it 'should raise a RecordInvalidForTier error if tier validation fails' do
        lambda{account_class.create_with_user_validation!}.should raise_error(ActiveRecord::RecordInvalidForTier)
      end
    end

    describe '#save_with_[tier]_validation' do
      it 'should return true if instance is saved with tier validation' do
        account_class.new(:expiration_date => Time.now).save_with_user_validation.should be_true
      end
  
      it 'should return false if tier validation fails' do
        account_class.new.save_with_user_validation.should be_false
      end
    end
    ...
    describe '#save_with_[tier]_validation!' do
      ...
      it 'should raise a RecordInvalidForTier error if tier validation fails' do
        lambda{account_class.new.save_with_user_validation!}.should raise_error(ActiveRecord::RecordInvalidForTier)
      end
    end
  
    describe '#valid_for_[tier]?' do
      it 'should return false if tier validation fails' do
        account_class.new(:expiration_date => Time.now).valid_for_user?.should be_true
      end
  
      it 'should return false if tier validation fails' do
        account_class.new.valid_for_user?.should be_false
      end
    end
  
    describe '#invalid_for_[tier]?' do
      it 'should return false if tier validation passes' do
        account_class.new(:expiration_date => Time.now).invalid_for_user?.should be_false
      end
  
      it 'should return true if tier validation fails' do
        account_class.new.invalid_for_user?.should be_true
      end
    end
  end
  ...
  describe 'Non-exclusive validation tier' do
    it 'should share validations with the standard validation chain' do
      account = account_class.new(:number => 'AAAAAA')
  ...
  describe 'Validation tier with included tier' do
    it 'should use included tier validations' do
      account = account_class.new(:preferred => true)
  ...
  describe 'Validation tier' do
    it 'should use on create option' do
      account = account_class.new(:number => '1238')
    ...
    it 'should use on update option' do
      account = account_class.new(:number => '1234')
      account.save
      account.should_not be_valid_for_pirate
    end
  
    it 'should include any default validations' do
      account = account_class.new(:number => '1ab', :expiration_date => 2.days.from_now)
      account.should_not be_valid_for_user
    end
  end
  ...
  describe 'Validation tier' do
    def build_account
      account = account_class.new(:balance => 50, :eula => true, :number => '123456',
...
describe Account do
  ...
  it_should_behave_like 'a model with tiered validation, or one of its subclass'
end
